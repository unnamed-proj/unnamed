...
const SubmitClass = () => {
  if (EmailMethod.value=="true" && CodeMethod.value=="true" && PASSWORDMethod.value=="true") return true;
  else return false;
}
...
function SendCode() {
  ...
  if (EmailMethod.value=="true") {
    ...
    axios.post(
        "https://alpha.unnamed.org.cn/api/user/password/email",{
          email: Email.value,
...
function Submit() {
  if (SubmitClass()) {
    axios.put("https://alpha.unnamed.org.cn/api/user/password",{
      "email": Email.value,
...
<template>
  <Center class="w-full h-screen px-4 flex-col gap-5">
    <div class="text-3xl mb-4 uppercase font-mono">Reset pwd on Unnamed-Proj.</div>
